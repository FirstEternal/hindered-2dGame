...
internal class Phase3_BossEnemy_DragonicFroster : IPhase
{
    ...
    private Attack_RainFromAbove attackMethod3;
    ...
    public void CreateVisuals(GameObject parent)
    {
        ...
        int frameCount = 5;
        Rectangle[] sourceRectangles = JSON_Manager.GetEnemiesSourceRectangles("Dragonic Froster_P3", frameCount);
        float[] frameTimers = [0.5f, 0.5f, 0.5f, 0.5f, 0.5f];
        animatedSprite = new SpriteAnimated(
                ...
                origins: JSON_Manager.GetEnemiesOrigin("Dragonic Froster_P3", frameCount, parent.transform.globalScale)
        );
    ...
    public void BeginPhase(BossEnemy bossEnemy, List<IBossMethod> movementMethods, List<IBossMethod> attackMethods)
    {
        ...
        bossEnemy.gameObject.transform.globalPosition = bossEnemy.ArrivalPosition;
        Player.Instance.gameObject.transform.globalPosition = new Vector2(
            bossEnemy.ArrivalPosition.X,
            bossEnemy.ArrivalPosition.Y + 11 * GameConstantsAndValues.SQUARE_TILE_WIDTH
        );
        ...
        attackMethod3 = (Attack_RainFromAbove)attackMethods[2];
        ...
        hasProjectileTerrainImunity = true;

        float fallHeight = 350;
        bool reverseSpread = false;
        attackMethod3.SetParameters(
            projectileDeathTimer,
            projectileType,
            projectileScale,
            startPosX,
    ...
    public void UpdatePhase(GameTime gameTime)
    {
        ...
        else if (animatedSprite.currFrameIndex == 4)
        {
            if (!isRainFromAboveReloading)
            {
                attackMethod3?.Execute();
